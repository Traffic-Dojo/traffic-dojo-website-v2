...
<template>
  <button
    ...
    :disabled="disabled"
    :class="[
      '-order-1 sm:order-1 w-fit flex items-center gap-4 rounded-full px-6 py-2',
      {
        'bg-modalSecondaryGrey text-textLight opacity-100': disabled,
        'bg-accent text-textDark': !disabled
      }
    ]"
    @click="!disabled && goToNextStep()"
  >
