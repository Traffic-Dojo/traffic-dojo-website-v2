<script setup lang="ts">
import AppSection from "../components/AppSection.vue";
import { motion, stagger, type VariantType } from "motion-v";

const contentVariants: Record<string, VariantType> = {
  hidden: {
    opacity: 0,
    x: -40,
    transition: { duration: 1.5 },
  },
  visible: {
    opacity: 1,
    x: 0,
    transition: {
      duration: 1.5,
      delayChildren: stagger(0.5),
    },
  },
};
</script>

<template>
  <AppSection
    class-name="flex flex-col justify-center gap-10 lg:grid lg:grid-cols-2 lg:items-center lg:gap-6"
  >
    <template #gradients>
      <motion.div
        class="spark absolute inset-0"
        :initial="{ opacity: 0 }"
        :while-in-view="{ opacity: 1, transition: { duration: 2, delay: 1.6 } }"
      ></motion.div>
      <motion.div
        class="colors absolute inset-0"
        :initial="{ opacity: 0 }"
        :while-in-view="{ opacity: 1, transition: { duration: 2 } }"
      ></motion.div>
      <motion.div
        class="earth absolute inset-0"
        :initial="{ opacity: 0 }"
        :while-in-view="{ opacity: 1, transition: { duration: 2 } }"
      ></motion.div>
    </template>

    <motion.div
      class="flex flex-col gap-3 font-medium lg:gap-12"
      :variants="contentVariants"
      initial="hidden"
      while-in-view="visible"
    >
      <motion.span
        class="text-2xl sm:text-3xl lg:text-4xl"
        :variants="contentVariants"
        >Our purpose:</motion.span
      >
      <motion.h2
        class="text-6xl sm:text-7xl lg:text-9xl"
        :variants="contentVariants"
      >
        Performing <br />
        marketing,
      </motion.h2>
      <motion.span
        class="text-textGray text-2xl lg:text-5xl"
        :variants="contentVariants"
        >not just launching ads</motion.span
      >
    </motion.div>

    <div
      class="relative mx-auto flex w-fit max-w-[650px] items-center justify-center px-[15%] sm:h-auto lg:w-full lg:px-0 [&>div]:absolute [&>div]:aspect-square"
    >
      <motion.div
        :initial="{ opacity: 0, transition: { duration: 1.6 } }"
        :while-in-view="{
          opacity: 1,
          transition: {
            duration: 1.6,
            repeat: Infinity,
            repeatType: 'reverse',
            delay: 1.6,
          },
        }"
        class="star top-[15%] left-[3%] -rotate-[20deg]"
      >
        <img src="/images/star1.png" alt="Google Analytics star" />
      </motion.div>

      <motion.div
        class="star top-0 right-0 z-10 -rotate-3"
        :initial="{ opacity: 0, transition: { duration: 1 } }"
        :while-in-view="{
          opacity: 1,
          transition: {
            duration: 1,
            repeat: Infinity,
            repeatType: 'reverse',
            delay: 1.6,
          },
        }"
      >
        <img src="/images/star3.png" alt="Click star" />
      </motion.div>

      <motion.div
        class="star bottom-[20%] left-0 rotate-[15deg]"
        :initial="{ opacity: 0, transition: { duration: 2 } }"
        :while-in-view="{
          opacity: 1,
          transition: {
            duration: 2,
            repeat: Infinity,
            repeatType: 'reverse',
            delay: 1.6,
          },
        }"
      >
        <img src="/images/star4.png" alt="Facebook star" />
      </motion.div>

      <motion.div
        class="star right-[15%] bottom-[10%] scale-150 -rotate-12"
        :initial="{ opacity: 0, transition: { duration: 1.8 } }"
        :while-in-view="{
          opacity: 1,
          transition: {
            duration: 1.2,
            repeat: Infinity,
            repeatType: 'reverse',
            delay: 1.6,
          },
        }"
      >
        <img src="/images/star5.png" alt="Google Ads star" />
      </motion.div>

      <motion.div
        class="star right-0 bottom-[40%] scale-90 -rotate-12"
        :initial="{ opacity: 0, transition: { duration: 1.8 } }"
        :while-in-view="{
          opacity: 1,
          transition: {
            duration: 1.8,
            repeat: Infinity,
            repeatType: 'reverse',
            delay: 1.6,
          },
        }"
      >
        <img src="/images/star2.png" alt="Dollar sign star" />
      </motion.div>

      <motion.video
        playsinline
        loop
        autoplay
        muted
        :controls="false"
        class="aspect-[1/2] max-h-[50svh] rotate-[30deg] lg:max-h-[75svh]"
        :initial="{ opacity: 0, x: -100, y: 300 }"
        :while-in-view="{ opacity: 1, x: 0, y: 0, transition: { duration: 2 } }"
      >
        <source src="/video/rocket.mov" type="video/quicktime" />
        <source src="/video/rocket.webm" type="video/webm" />
      </motion.video>
    </div>
  </AppSection>
</template>

<style scoped>
.spark {
  z-index: 999999999;
  transform: translateY(8svh) translateX(40%);
  background: url("/images/spark.png") no-repeat center bottom / contain;
  mix-blend-mode: lighten;
}

.earth {
  mix-blend-mode: screen;
  background: url("/images/earth.png") no-repeat 50% 105% / contain;
  /* background: url("/images/earth.png") no-repeat 50% 105% / contain; */
}

.colors {
  --size: calc(min(200svh, calc((100svh + 100svw) / 0.3)) * 2);

  width: var(--size);
  height: var(--size);

  position: absolute;
  top: 0%;
  left: 50%;

  transform: translateX(-30%) translateY(-60%);

  background: radial-gradient(
    50% 50% at 50% 50%,
    rgba(17, 96, 251, 1) 0%,
    rgba(24, 24, 24, 1) 100%
  );
}

.star {
  width: clamp(80px, 2.0916rem + 8.1425vw, 126px);
}

@media screen and (max-width: 1024px) {
  .spark {
    display: none;
  }

  .earth {
    background: url("/images/earth_sm.png") no-repeat 50% 30svh / contain;
  }

  .colors {
    inset: unset;
    top: 0;
    left: 0;
    width: 200svh;
    height: 100svh;
    transform: translateY(0%) translateX(-10%);
    background:
      radial-gradient(
        50% 60% at 65% 50%,
        rgba(236, 204, 255, 0.5) 0%,
        rgba(24, 24, 24, 0) 100%
      ),
      radial-gradient(
        100% 70% at 80% 50%,
        rgba(17, 96, 251, 1) 0%,
        rgba(24, 24, 24, 1) 100%
      );
  }
}
</style>
